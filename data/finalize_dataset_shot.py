...
# ------------------------------
# Collect features (and record clip basenames)
# ------------------------------

def collect_pose_features(root: Path, out_dir: Path) -> Set[str]:
    """
    Copy all *.npy from:
      shot/features/<video>/ShotEvent/<child>/<pid>/*.npy
    into:
      shot/final/pose_features/<basename>.npy

    Returns a set of basenames (without extension) that were copied.
    """
    src_root = root / "shot" / "features"
    ...
    for video_dir in sorted(src_root.iterdir()):
        if not video_dir.is_dir():
            continue
        se_dir = video_dir / "ShotEvent"
        if not se_dir.exists():
            continue
        for child_dir in sorted(se_dir.iterdir()):
            if not child_dir.is_dir():
                continue
            for pid_dir in sorted(child_dir.iterdir()):
                ...
                for npy_path in sorted(pid_dir.glob("*.npy")):
                    ...
                    try:
                        ...
                        copied.add(npy_path.stem)  # basename without .npy
                    except Exception as e:
                        print(f"[ERROR] Copy failed {npy_path} -> {dest}: {e}")
    return copied

# ------------------------------
# Choose Train/Validation parents (upmost parent video)
# ------------------------------
...
def pick_parent_split(
    ...
    """
    Build a deterministic Train/Validation split over *upmost parent videos* under:
      shot/label/tal/<video>/ShotEvent/<child>/<pid>/*.json
    ...
    print(f"Upmost parents considered: {len(parents_considered)} "
          f"(Train={len(train_parents)}, Validation={len(valid_parents)})")
    return train_parents, valid_parents

# ------------------------------
# Merge TAL JSONs (filtered & subset reassigned by parent)
# ------------------------------
...
def merge_tal_jsons_filtered(
    ...
    """
    Merge clip jsons from:
      shot/label/tal/<video>/ShotEvent/<child>/<pid>/*.json (skip match.json)
    ONLY include those whose filename stem is in valid_clip_names (i.e., has matching npy).
    Reassign each clip's subset based on its upmost parent video:
      - parent in train_parents  -> "Train"
      - parent in valid_parents  -> "Validation"
    """
        ...
        for child_dir in sorted(se_dir.iterdir()):
            ...
            for pid_dir in sorted(child_dir.iterdir()):
                ...
                for jp in sorted(pid_dir.glob("*.json")):
                    if jp.name.lower() == "match.json":
                        continue
                    stem = jp.stem  # e.g., child_pid_clipid
                    if stem not in valid_clip_names:
                    ...
                    for k, v in data.items():
                        # Prefer only the key that matches filename; allow others if they also match.
                        if k != stem and k not in valid_clip_names:
                            continue
                        # Reassign subset (defensively update both 'subset' and 'Subset')
...
def main():
    parser = argparse.ArgumentParser(
        description="Collect pose features and merge TAL JSONs for ShotEvent hierarchy (filtered to matching clips) with Train/Validation split by upmost parent video."
    )
    parser.add_argument("--root", type=str, default=".", help="Project root containing 'shot/'.")
    parser.add_argument("--version", type=str, default="Shot-60fps", help="Version string for the merged annotations JSON.")
    parser.add_argument("--features_out_dir", type=str, default="shot/final/pose_features",
                        help="Destination folder to collect all .npy features.")
    parser.add_argument("--annotations_out_dir", type=str, default="shot/final/annotations",
                        help="Destination folder for the merged annotations JSON.")
    parser.add_argument("--annotations_filename", type=str, default="shot.json",
                        help="Filename for merged annotations (default: 'shot.json').")
    parser.add_argument("--train_ratio", type=float, default=0.8,
                        help="Fraction of upmost parent videos to assign to Train (rest to Validation).")
    parser.add_argument("--seed", type=int, default=42,
                        help="Random seed for deterministic parent split.")
    ...
    ann_out_path = ann_out_dir / (args.annotations_filename or f"{args.version}.json")
    ...
    print("\n== Splitting upmost parent videos into Train/Validation ==")
    ...
    print("\n== Merging TAL annotations (filtered + subset reassigned by parent) ==")
