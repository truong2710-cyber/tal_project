...
def _init_dist_if_needed():
    """Init torch.distributed if launched via torchrun; return (rank, world_size, local_rank, is_dist)."""
    is_dist = all(k in os.environ for k in ("RANK", "WORLD_SIZE", "LOCAL_RANK"))
    if not is_dist:
        return 0, 1, 0, False
...
def _shard_list(xs, rank, world_size):
    # strided sharding; guarantees disjoint partitions even if len(xs) % world_size != 0
    return xs[rank::world_size]


def main():
    init_args, call_args, display_alias = parse_args()
    input_list = list(call_args['inputs'])  # ensure list (not generator)
    ...
    rank, world_size, local_rank, is_dist = _init_dist_if_needed()

    # Each rank gets its own subset
    my_inputs = _shard_list(input_list, rank, world_size)

    # Build one inferencer per rank/process
    inferencer = MMPoseInferencer(**init_args)

    # Progress bar per-rank (keeps logs readable). Rank 0 usually has ~ceil(n/world)
    iterator = tqdm.tqdm(my_inputs, desc=f'Processing (rank {rank}/{world_size})') if my_inputs else my_inputs

    for inp in iterator:
        # Avoid mutating the shared dict across iterations/ranks
        one_call = copy.deepcopy(call_args)
        one_call['inputs'] = inp

        results = []
        for result in inferencer(**one_call):
            # Extend with predictions from this step
            results.extend(result['predictions'])

        # Build save path mirroring your original logic
        save_path = inp.replace('frame', 'keypoints')
        if inp.endswith(('mp4',)):
